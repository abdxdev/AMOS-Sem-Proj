...
public sealed partial class PizzaPopupTemplate : ContentDialog
{
    ...
    public void LoadToppings()
    {
        try
        {
            var reader = App.DatabaseService!.get_product_by_category_and_subcategory("topping", null);
            while (reader.Read())
            {
    ...
    public async void ContentDialog_PrimaryButtonClick(ContentDialog sender, ContentDialogButtonClickEventArgs args)
    {
        ...
        try
        {
            App.DatabaseService.insert_an_order(tableId, itemId, quantity, finalPrice, estimatedTime, description);
        }
        catch (Exception ex)
        {
            Debug.WriteLine(ex.Message);
